...
def plot_sla_breach_by_state(logistics_df, contingency, chi2, p, dof):
    ...
    state_sla_rates.plot(kind='barh', width=0.85, color=colors, edgecolor='white', ax=ax) 
    
    ax.axvline(national_sla_rate, color='r', linestyle='--', alpha=0.7,
               label=f"National Rate: {national_sla_rate:.2f}%\nChi2:{chi2:.0f}, p={p:.2e}, dof={dof}")
    ax.set_xticks(np.arange(0, 25, 2)) 
    ax.set_title('SLA Breach Rate by Customer State (%)', fontsize=11, fontweight='bold') 
...
def plot_logistics_correlation(logistics_df: pd.DataFrame):
    ...
    logistics_feature = ['is_interstate', 'is_late', 'vendor_handling_days', 
                         'carrier_transit_days', 'distance_km', 'mean_review_score', 
                         'total_freight_value']
    correlation_matrix = logistics_df[logistics_feature].corr(method='spearman')
    
    sns.heatmap(correlation_matrix, cmap='coolwarm', square=True, annot=True, 
                vmin=-1, vmax=1, linewidths=0.5, fmt=".2f", ax=ax) 
    
    ax.set_title('Logistics Drivers: Spearman Correlation Matrix', fontsize=11, fontweight='bold') 
...
def plot_logistics_bottlenecks(logistics_df: pd.DataFrame):
    ...
    logistics_df[['carrier_transit_days', 'vendor_handling_days']].mean().plot(
        kind='bar', ax=axis[0], color=['teal', 'salmon']
    )
    ...
    for v, c in colors.items():
        ...
        subset.plot(x='distance_km', y='total_freight_value', color=c, 
                    kind='scatter', alpha=0.3, ax=axis[1], label=f'Interstate: {v}')
